...
extern "C"
{
  ...
  /**
   * @brief Exception and interrupt handler declarations.
   *
   * These handlers are placed in the interrupt vector table. Unless overridden
   * by the application, each handler is defined as a weak alias to
   * Default_Handler.
   */
  ...
  /**
   * @brief Interrupt table in Memory.
   * 
   * Upon any called interrupt, the function that is written to the corresponding
   * vector address is called and executed. These functions are the ones implemented above.
   * the actual table is written to memory in the linker file ls.ld.
   */
  __attribute__((section(".isr_vector"), aligned(512), used))
